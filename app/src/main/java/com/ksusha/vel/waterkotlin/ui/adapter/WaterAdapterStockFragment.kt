...
class WaterAdapterStockFragment(
    private val listener: RecyclerviewOnClickListener,
    private val waters: List<WaterEntity>
) :
    RecyclerView.Adapter<WaterAdapterStockFragment.WaterViewHolder>() {
        ...
        holder.waterItemBinding.waterImage.setOnClickListener {
            ...
            intent.putExtra(
                "waterPrice", String.format(
                    "Ціна: %s,00грн",
                    waters[position].prise
                )
            )
            intent.putExtra("waterCount", waters[position].count.toString())
